<template>
  <div class="fi-main-report">
    <router-link v-if="checkLeader()" to="/flexible/leader/approve" class="fi-link-back">
...
export default {
  ...
  methods: {
    activeTab(routerName) {
      if (routerName == "report_approved_all") {
        this.tabActive = ["", "active", ""];
      } else if (routerName == "report_rejected_all") {
        this.tabActive = ["", "", "active"];
      } else {
        this.tabActive = ["active", "", ""];
      }
      ...
      if (i == 1) {
        this.$router.push({
          path:
            ...
            "/flexible/report/approve/all"
        });
      } else if (i == 2) {
        this.$router.push({
          path:
            this.$store.getters["flexible_interest_module/root"] +
            "/flexible/report/reject/all"
        });
      } else {
        this.$router.push({
          path:
