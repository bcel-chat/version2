<template>
    <div class="fi-main-leader">
        <a href="javascript:event" class="fi-switch-on-off">
            ...
            </label> <span class="fi-toggle-status">{{on_off[0]?'ຮັບອະນຸມັດ':'ບໍ່ຮັບອະນຸມັດ'}}</span>
        </a>
        <router-link to="/flexible/report/pending/all" class="fi-link-report"><i class="fa fa-line-chart"></i></router-link>
        <ul class="nav nav-tabs  mb-3 justify-content-center hide-on-print-mode">
            ...
            <li class="nav-item">
                <a class="nav-link" href="javascript:event" :class="tabActive[1]" @click="changeActiveLeader(1)"><strong class="fi-tab-title">HISTORY</strong></a>
            </li>
        </ul>
        <div class="content">
            <router-view/>
...
export default {
  ...
  created(){
        ds.rpc.make('/bcel/api/flexible/interest/get/user/on/off', { userId: this.$store.getters['flexible_interest_module/user']}, ( error, result1 ) => {
  ...
  methods: {
    ...
    changeOnOffLeader(){
        ds.rpc.make('/bcel/api/flexible/interest/update/user/on/off', { userId: this.$store.getters['flexible_interest_module/user'], on_off: this.on_off[0]?'ON':'OFF'}, ( error, result1 ) => {
            if(result1)
            {
                //console.log(result1);
            }
