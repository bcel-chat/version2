  ...
  <div class="container">
    ...
    <div class="fi-add-user-content" v-if="checkSaveLoading">
      ...
      <div class="fi-processing-container text-center">
        <img
          src="@/assets/flexible_interest/images/ic-processing.gif"
          style="width: 60px; filter:brightness(50%);"
          alt="Processing"
        />
        <br />
    ...
    <div class="row hide-on-print-mode">
      ...
      <div class="col-12 col-sm-12 col-md-12 col-lg-12 text-left">
        <div class="fi-edit-content" style="color: rgba(247, 82, 6, 0.959);">
          ...
          <div
            class="fi-response-edit-body text-left"
            v-for="(edit, index) in editResponse"
            :key="index"
          >
            <div class="text-left">
              <div>
                <strong
                  style="border-bottom: 1px gray solid; font-size: 18px;"
                >{{edit['requestUserName']}}:</strong>
              </div>
              <div style="margin-top:-1px; font-size: 12px; color: #908C84;">
                <span>{{edit['position']}} {{edit['divisionName']}} {{edit['departmentName']}}</span>
              </div>
            </div>
            <div>
              &nbsp&nbsp&nbsp&nbsp
            ...
            <div class="row">
                  ...
                  <div class="col-4 text-left">
                    ...
                    <label class="fi-container">
                      ...
                      <input
                        ...
                        name="radio"
                        value="TAI"
                        v-model="customer_info['customerType']"
                        @change="validateType"
                      />
              ...
              <div class="col-12 col-sm-12 col-md-6 col-lg-6">
                ...
                <input
                  type="text"
                  required
                  v-model="customer_info['customerName']"
                  @change="validateName"
                />
                <div class="error-message" v-if="error_validation['name'].length > 0">
                  ...
                  <span
                    class="danger"
                    v-for="(error, index) in error_validation['name']"
                    :key="index"
                  >{{error}}</span>
                </div>
              </div>
              <div class="col-12 col-sm-12 col-md-6 col-lg-6">
                ...
                <input
                  type="text"
                  required
                  v-model="customer_info['customerTel']"
                  @change="validateTel"
                />
                <div class="error-message" v-if="error_validation['tel'].length > 0">
                  <span
                    class="danger"
                    v-for="(error, index) in error_validation['tel']"
                    :key="index"
                  >. {{error}}</span>
                </div>
              </div>
              ...
              <div class="col-12">
                ...
                <div
                  class="fi-account-group"
                  v-for="(account, index) in customer_info['bankAccounts']"
                  :key="index"
                >
                  ...
                  <strong
                    class="delIcon"
                    v-if="customer_info['bankAccounts'].length > 1"
                    @click="deleteAccount(index, account)"
                  >x</strong>
                  <div class="row">
                    <div class="col-12 col-sm-12 col-md-6 col-lg-6">
          ...
          <div class="row">
            <div class="col-12 col-sm-12 col-md-6 col-lg-6">
              ...
              <input
                type="text"
                required
                v-model="customer_info['depositAmount']"
              ...
              <div class="error-message" v-if="error_validation['deposit_amount'].length > 0">
                <span
                  class="danger"
                  v-for="(error, index) in error_validation['deposit_amount']"
                  :key="index"
                >{{error}}</span>
              </div>
            </div>
            <div class="col-12 col-sm-12 col-md-6 col-lg-6">
              ...
              <select
                id="currency"
                required
                v-model="customer_info['currencyId']"
              ...
              >
                ...
                >{{currency['currencyNameLa']}} ({{currency['currencyCode']}})</option>
              </select>
              <div class="error-message" v-if="error_validation['currency'].length > 0">
                <span
                  class="danger"
                  v-for="(error, index) in error_validation['currency']"
                  :key="index"
                >{{error}}</span>
              </div>
            </div>
            <div class="col-12 col-sm-12 col-md-6 col-lg-6">
              ...
              <select
                id="withdraw_age"
                v-model="customer_info['depositType']"
                required
              ...
              <div class="error-message" v-if="error_validation['deposit_age'].length > 0">
                <span
                  class="danger"
                  v-for="(error, index) in error_validation['deposit_age']"
                  :key="index"
                >{{error}}</span>
              </div>
            </div>
            <div class="col-12 col-sm-12 col-md-6 col-lg-6">
              ...
              <div class="text-right fi-dropdown-panel" @mouseleave="unhoverDropdownPanel()">
                <span
                  class="fi-dropdown-icon"
                  @click="(aclasses.length>0)?show=!show:show=false"
                >&#9660;</span>
                <input
                  type="text"
                  :class="customer_info['interestChosen']?'fi-valid': 'fi-invalid'"
                  v-model="customer_info['interestChosen']"
                  required
                  placeholder="ເລືອກໄລຍະຮັບດອກເບ້ຍ"
                  readonly
                  @click="(aclasses.length>0)?show=!show:show=false"
                />
                <!--  -->
                <ul
                  v-show="show"
                  id="interest"
                  class="text-left fi-dropdown"
                  @mouseover="hoverDropdownPanel()"
                >
                  <li class="fi-placehoder" @click="aclassSelection({})">ເລືອກໄລຍະຮັບດອກເບ້ຍ</li>
                  <li
                    v-for="(aclass, index) in aclasses"
                    :key="index"
                    :class="hover?'':dropdownAcive(aclass)"
                    @click="aclassSelection(aclass)"
                  >{{aclass['descriptionLao']}}</li>
                </ul>
                <div class="error-message" v-if="error_validation['aclass'].length > 0">
                  <span
                    class="danger"
                    v-for="(error, index) in error_validation['aclass']"
                    :key="index"
                  >{{error}}</span>
                </div>
              </div>
            ...
            <div class="col-12 col-sm-12 col-md-6 col-lg-6">
              ...
              <div
                class="_valid"
              >{{(customer_info['currencyCode'] == 'USD')?customer_info['currencyUnit']:''}}{{getEarningPerYear('normalInterest')}}{{(customer_info['currencyCode'] != 'USD')?customer_info['currencyUnit']:''}}</div>
            </div>
            <div class="col-12 col-sm-12 col-md-6 col-lg-6">
            ...
            <div class="col-12 col-sm-12 col-md-6 col-lg-6">
              ...
              <span
                style="color: #20A816; font-size: 12px;"
            ...
            <div class="col-12 col-sm-12 col-md-6 col-lg-6">
              ...
              <div
                class="_valid"
              >{{(customer_info['currencyCode'] == 'USD')?customer_info['currencyUnit']:''}}{{getEarningPerYear('customerInterest')}}{{(customer_info['currencyCode'] != 'USD')?customer_info['currencyUnit']:''}}</div>
            </div>
          </div>
        </div>
        <div class="fi-card">
          ...
          <div>
            <div class="row">
              <div class="col-12">
                ...
                <textarea
                  required
                  v-model="customer_info['staffNote']"
                  rows="4"
                ...
                <div class="error-message" v-if="error_validation['staff_note'].length > 0">
                  <span
                    class="danger"
                    v-for="(error, index) in error_validation['staff_note']"
                    :key="index"
                  >{{error}}</span>
                </div>
              </div>
              <div class="col-12">
                ...
                />
                <ul class="fi-text">
                  <li v-for="(new_file, new_index) in new_files" :key="'new' + new_index">
                    {{new_file['name']}}
                    <span class="closeIcon" @click="removeFile(new_index)">x</span>
                  </li>
                  <li v-for="(file, index) in document_files" :key="index">
                    <a
                      :href="fileIp + file['docUrl']"
                      target="_blank"
                      rel="noopener noreferrer"
                    >{{file['docName']}}</a>
                    <span class="closeIcon" @click="deleleFile(index, file)">x</span>
                  </li>
                </ul>
              </div>
            </div>
...
export default {
  ...
  data() {
    return {
      ...
      customer_info: this.$store.getters[
        "flexible_interest_module/customer_detail"
      ],
      document_files: [],
      new_files: [],
      editResponse: [],
      products: this.$store.getters["flexible_interest_module/customer_detail"][
        "products"
      ],
      currencies: [],
      depositTypes: [],
  ...
  watch: {
    $route(to, from) {
      ds.rpc.make(
        "/bcel/api/flexible/interest/customer/requirement/by/id",
        { requirementId: atob(this.$route.params["id"]) },
        (error, result) => {
          if (result) {
            //console.log(result);
            this.customer_info = result;
            this.customer_info["depositAmount"] = this.customer_info[
              "depositAmount"
            ]
              .toLocaleString()
              .replace(/,/g, ".");
            if (this.checkTrueEditData()) {
              ...
              axios
                .get(
                  address["serverIp"] +
                    ...
                    "/" +
                    this.customer_info["currencyCode"] +
                    "/" +
                    this.customer_info["requirementId"]
                )
                .then(res => {
                  this.customer_info["normalInterest"] =
              ...
              for (
                var i = 0;
                i < this.customer_info["bankAccounts"].length;
                i++
              ) {
  ...
  methods: {
    ...
    checkTrueEditData() {
      return (
        this.customer_info["createUser"] ==
          this.$store.getters["flexible_interest_module/user"] &&
        this.customer_info["editCount"] > 0
      );
    },
    validateAccounts() {
    ...
    validateAccountNo(i) {
      ...
      if (
        this.customer_info["bankAccounts"][i]["accountNo"].trim().length != 18
      ) {
        if (
          this.error_validation["bank_accounts"][i]["acc_no"].indexOf(
            "ເລກບັນຊີທະນາຄານຕ້ອງເປັນຕົວເລກ"
    ...
    addAccount() {
      this.customer_info["bankAccounts"].push({
        id: "0",
        accountNo: "",
        description: "",
    ...
    deleteAccount(i, account) {
      if (account["id"] == 0) {
        ...
        ds.rpc.make(
          "/bcel/api/flexible/interest/customer/requirement/delete/bank/account",
          { _id: account["id"] },
          (error, result) => {
            if (result) {
              this.customer_info["bankAccounts"].splice(i, 1);
    ...
    deleleFile(i, doc) {
      ds.rpc.make(
        "/bcel/api/flexible/interest/customer/requirement/delete/document",
        doc,
        (error, result) => {
          if (result) {
      ...
      if (this.customer_info["currencyId"].trim()) {
        for (var i = 0; i < this.currencies.length; i++) {
          if (
            ...
            this.customer_info["currencyId"].trim()
          ) {
      ...
      if (this.customer_info["depositType"].trim()) {
        for (var i = 0; i < this.depositTypes.length; i++) {
          if (
            ...
            this.customer_info["depositType"].trim()
          ) {
            this.customer_info["depositTypeName"] = this.depositTypes[i][
              "typeName"
            ];
            this.customer_info["depositTotalDays"] = this.depositTypes[i][
              "totalDays"
            ];
            break;
          }
      ...
      if (
        ...
        ds.rpc.make(
          "/bcel/chat/api/flexible/interest/max/flexible",
          {
            ...
            customerDeposit: this.customer_info["depositAmount"]
              .trim()
              .replace(/\./g, ""),
    ...
    validateCustomerDepositAmount() {
      if (this.customer_info["depositAmount"].trim()) {
        ...
        this.customer_info["depositAmount"] = Number(amount)
          .toLocaleString()
          .replace(/,/g, ".");
        this.customer_info["depositAmount"] +=
    ...
    getGeneralInterest() {
      if (
        ...
        ds.rpc.make(
          "/bcel/api/flexible/interest/general",
          {
            currencyCode: this.customer_info["currencyCode"].trim(),
            acClass: this.customer_info["accountClass"]
          },
          (error, result) => {
            if (error) {
              this.customer_info["normalInterest"] = 0;
    ...
    getAClasses() {
      if (this.customer_info["depositType"].trim()) {
        ds.rpc.make(
          ...
          {
            depositAge: this.customer_info["depositType"].trim(),
            customerType: this.customer_info["customerType"]
          },
          (error, result) => {
            if (error) {
              this.aclasses = [];
    ...
    getEarningPerYear(key) {
      if (this.customer_info["depositAmount"]) {
        var amount1 = this.customer_info["depositAmount"]
          .toString()
          .replace(/\./g, "");
        ...
        if (
          !isNaN(amountFloat) &&
          this.customer_info["currencyCode"] &&
          this.customer_info["depositType"]
        ) {
          var interest =
            amountFloat *
            (this.customer_info[key] / 100) *
            (this.customer_info["depositTotalDays"] / 360);
          return interest.toLocaleString(undefined, {
    ...
    getExceededOutFlexible() {
      try {
        var customer_interest = parseFloat(
          this.customer_info["customerInterest"]
        );
        var max_interest =
          parseFloat(this.customer_info["normalInterest"]) +
          parseFloat(this.customer_info["interestFlexible"]);
        if (
    ...
    getExceededInFlexible() {
      try {
        ...
        var max_interest =
          normal_interest + parseFloat(this.customer_info["interestFlexible"]);
        if (
          customer_interest <= max_interest &&
    ...
    getRelationData() {
      ds.rpc.make(
        "/bcel/api/flexible/interest/customer/requirement/document/attach",
        { requirementId: atob(this.$route.params["id"]) },
        (error, result) => {
          if (result) {
      ...
      ds.rpc.make(
        "/bcel/api/flexible/interest/edit/notification",
        { requirementId: atob(this.$route.params["id"]) },
        (error, result) => {
          if (result) {
    ...
    saveNewData() {
      ...
      if (this.editComment.trim()) {
        ...
        if (this.new_files.length > 0) {
          ...
          axios
            .post(
              address["serverIp"] + "/bcel/api/flexible/interest/file/upload",
              formData,
              {
                headers: {
                  "content-type": "multipart/form-data"
          ...
          this.customer_info["files"] = [];
          this.customer_info["user"] = this.$store.getters[
            "flexible_interest_module/user"
          ];
          this.updateRequirement(this.customer_info);
        }
    ...
    updateRequirement(data) {
      ...
      ds.rpc.make(
        "/bcel/chat/api/flexible/interest/customer/requirement/update",
        data,
        (error, result) => {
...
input.fi-account-input:focus,
input.fi-account-input:valid {
