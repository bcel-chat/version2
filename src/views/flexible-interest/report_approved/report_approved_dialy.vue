                        ...
                        <table class="table text-left">
                            ...
                            <tbody>
                                <tr v-for="(requirement, index) in customer_requirments" :key="index">
                                    ...
                                    <td :class="getExceededInFlexible(requirement['customerInterest'], requirement['normalInterest'], requirement['interestFlexible'])?'in-scope':getExceededOutFlexible(requirement['customerInterest'], requirement['normalInterest'], requirement['interestFlexible'])?'out-scope':''">{{requirement['customerInterest']}}%</td>
                                    ...
                                    <td>
                                        <span v-if="(requirement['approveStatus'] == 'APPROVED')" :class="requirement['approveStatus']">&#10003;</span>
                                        <!-- &#9745; -->
                                        <span  v-else-if="(requirement['approveStatus'] == 'SUCCESS')" :class="requirement['approveStatus']">&#9733;</span>
                                        <span  v-else-if="(requirement['approveStatus'] == 'CANCEL')" :class="requirement['approveStatus']">&#215;</span>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
    ...
    methods: {
        ...
        downloadToExcel() {
            if(this.customer_requirments.length>0){
                import('@/vendor/Export2Excel').then(excel => {
                    const tHeader = ['ລະຫັດ', 'ຊື່ລູກຄ້າ', 'ອາຊີບ', 'ປະເພດ', 'ເບີໂທ', 'ຂໍ້ມູນອື່ນໆ', 'ຈຳນວນເງິນຝາກ', 'ສະກຸນເງິນ', 'ລະຫັດສະກຸນເງິນ', 'ດອກເບ້ຍທີ່ຕ້ອງການ', 'ໄລຍະຝາກ', 'ໄລຍະຮັບດອກເບ້ຍ', 'ດອກເບ້ຍປົກກະຕິ', 'ດອກເບ້ຍຍຶດຍຸ່ນສູງສຸດ', 'ດອກເບ້ຍ(ພະແນກບໍລິຫານທຶນ)', 'ດອກເບ້ຍອະນຸມັດ', 'ຜູ້ຂໍອະນຸມັດ', 'ຂະແໜງ', 'ພະແນກ', 'ສາຂາ', 'ຄຳຄີດເຫັນຂອງພະນັກງານ', 'ວັນທີ່', 'ເວລາ', 'ຜູ້ອະນຸມັດ', 'ວັນທີ່', 'ເວລາ', 'ສະຖານະ', 'ໝາຍເຫດ']
                    const filterVal = ['requirementId', 'customerName', 'customerOccupation', 'customerTypeDesc', 'customerTel', 'customerDetail', 'depositAmount', 'currencyName', 'currencyCode', 'customerInterest', 'depositTypeName', 'interestChosen', 'normalInterest', 'interestFlexible','bankInterest', 'approveInterest', 'createUserName', 'createUserDivision', 'createUserDepartment', 'createUserBranch', 'staffNote', 'createDate', 'createTime', 'approveUserName', 'approveDate', 'approveTime', 'approveStatus', 'cancleComment']
        ...
        getExceededOutFlexible(customerInterest, normalInterest, interestFlexible ) {
            try{
                ...
                var max_interest = parseFloat(normalInterest) + parseFloat(interestFlexible.toString());
                if((customer_interest > max_interest) && (normalInterest > 0) && (interestFlexible > 0)){
                    return true;
                } else {
                    return false
                }
        ...
        getExceededInFlexible(customerInterest, normalInterest, interestFlexible) {
            try{
                ...
                if((customer_interest <= max_interest) && (customer_interest > normal_interest) && (normalInterest > 0) && (interestFlexible > 0)){
                    return true;
                } else {
                    return false;
                }
...
.APPROVED, .fi-APPROVED, .SUCCESS, .fi-SUCCESS{
    color: #20A816;
    font-size: 18px;
    font-weight: bold;
}
