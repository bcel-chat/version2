...
export default {
  ...
  methods: {
    ...
    getAllPendingSearchLoadMore(){
        ds.rpc.make('/bcel/api/flexible/interest/customer/requirement/by/staff/pending/search', { 
    ...
    getAllPendingLoadMore(){
        ds.rpc.make('/bcel/api/flexible/interest/customer/requirement/by/staff/pending', { 
            staffUser:  this.$store.getters['flexible_interest_module/user'],
            rowIndex: this.customer_requirement_history.length
        }, ( error, result ) => {
            if(result){
    ...
    openEditDetail(history){
        ...
        //console.log(history);
        this.$store.commit('flexible_interest_module/addRequestApproveDetail', { info: history });
        this.$router.push({path: this.$store.getters['flexible_interest_module/root']+'/flexible/customer/requirement/edit/' + btoa(history['requirementId'])});
    },
