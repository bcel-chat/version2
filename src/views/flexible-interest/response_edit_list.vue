  ...
  <div class="container">
    <div class="row">
      <div class="col-12 col-sm-12 col-md-12" v-if="checkData()">
        <h3>
          <strong>ປະຫວັດຂໍອະນຸມັດດອກເບ້ຍຍືດຍຸນ</strong>
        </h3>
      </div>
      <div class="col-12" v-if="!checkData()">
        <div class="fi-not-found">
          <Strong>ບໍມີຂໍ້ມູນ</Strong>
        </div>
      </div>
      <div
      ...
      >
        <div class="fi-card text-left">
          <div class="fi-card-title">
            <h4>
              <span style="font-size: 16px;">ລູກຄ້າ:</span>
...
export default {
  ...
  created() {
    ds.rpc.make(
      "/bcel/api/flexible/interest/customer/requirement/by/staff",
      { staffUser: "SOMCHANH" },
      (error, result) => {
        if (error) {
          this.aclasses = [];
    ...
    getCustomerRequirementDetail(history) {
      ds.rpc.make(
        ...
        (error, result) => {
          ...
          this.$store.commit(
            "flexible_interest_module/addRequestApproveDetail",
            { info: history }
          );
