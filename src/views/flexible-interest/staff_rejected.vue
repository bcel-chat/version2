        ...
        <div v-if="showComment">
            ...
            <div class="fi-edit-dialog">
                ...
                <div class="fi-edit-footer">
                    <button type="button" class="fi-btn-edit-cancel" @click="cancelCustomerConfirm()">ລູກຄ້າຍົກເລີກ</button>
                ...
                <div class="fi-card text-left" :class="'BACKGROUND-'+history['approveStatus']">
                    <div class="fi-card-title">
                        ...
                        <span class="fi-top-right-action" v-if="(history['approveStatus'] == 'REJECTED_50')">
                            ...
                            <span title="ຢືນຢັນຄຳຕອບຂອງລູກຄ້າ" class="closeIcon" v-if="(history['approveStatus'] == 'REJECTED_50')" @click="showCommentDialog(history, index)">
                                <i class="fa fa-check-circle" aria-hidden="true"></i>
                            </span>
                        </span>
                    </div>
                        ...
                        <div class="fi-box">
                            <table>
                                <tbody>
                                    ...
                                    <tr>
                                        <th>ອະນຸມັດ: </th>
                                        <td :class="(history['approveStatus'] == 'REJECTED_50')?'REJECTED':history['approveStatus']">{{(history['approveStatus'] == 'REJECTED_50')?'REJECTED':history['approveStatus']}}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
  ...
  methods: {
    ...
    okCustomerConfirm(){
        if(this.comment.trim()){
            this.message = '';
            ds.rpc.make('/bcel/chat/api/flexible/interest/customer/requirement/reject50/confirm', { 
                requirementId: this.chosenRejectOKCancelRequirement['history']['requirementId'], 
                        ...
                        buttons: [
                            {
                                ...
                                action: () => {
                                    if(result['success']){
                                        this.customer_requirement_history.splice(this.chosenRejectOKCancelRequirement['index'], 1);
                        ...
                        buttons: [
                            {
                                ...
                                action: () => {
                                    if(result['success']){
                                        this.customer_requirement_history[this.chosenRejectOKCancelRequirement['index']]['approveStatus'] = 'REJECTED';
                                    }
                                    this.chosenRejectOKCancelRequirement = {};
                                    this.comment = "";
                                    this.fi_config['show'] = false;
    ...
    getAllRejectedSearchLoadMore(){
        ds.rpc.make('/bcel/api/flexible/interest/customer/requirement/by/staff/rejected/search', { 
    ...
    getAllRejectedLoadMore(){
        ds.rpc.make('/bcel/api/flexible/interest/customer/requirement/by/staff/rejected', { 
            staffUser:  this.$store.getters['flexible_interest_module/user'],
            rowIndex: this.customer_requirement_history.length
        }, ( error, result ) => {
            if(result){
