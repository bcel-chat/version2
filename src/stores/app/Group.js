...
const group = {
  ...
  getters: {
    ...
    _participantId(state) {
      return `${
        state.participantId.length > state.participantId.length
          ? state.participantId.length
          : 0
      } participants`;
    },
    _selected(state) {
  ...
  actions: {
    getRoomID({ commit }, payload) {
      commit("getRoomID", payload);
    },
    getAdminStatus({ commit }, payload) {
      commit("getAdminStatus", payload);
    },
    getParticipant({ commit }, payload) {
      ds.rpc.make("getParticipant", payload, (err, data) => {
        if (!err) commit("getParticipant", data);
        else console.log(err);
      });
    },
    getParticipantId({ commit }, payload) {
      ds.rpc.make("getParticipantId", payload, (err, data) => {
        if (!err) commit("getParticipantId", data);
        else console.log(err);
      });
    },
    getParticipantAdded({ commit }, payload) {
      ds.rpc.make("getParticipant", payload, (err, data) => {
        if (!err) commit("getParticipantAdded", data);
        else console.log(err);
      });
    },
    setGroup({ commit, dispatch }, payload) {
    ...
    setParticipant({ dispatch }, payload) {
    ...
    async setGroupPicture({ state, commit }, payload) {
      ...
      axios
        .post(`${process.env.VUE_APP_ACCESS_API}/profilepicture`, formData)
        .then(() => {
